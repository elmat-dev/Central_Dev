...
int enable_rtc(void) {
  return TM_RTC_Init(TM_RTC_ClockSource_External);
}
...
void TM_RTC_RequestHandler() {
  printf("Zg³oszeneie od  RTC \r\n");
  NVIC_SystemReset();
}


 int GSM_updateTime (TM_RTC_Time_t *time) 
 {
  int result=0;
  char tmp [32];
  char *token;
  ...
  {
    Delayms(100);
    if(GSM_AT_wConfirm("ATE0\r\n", "OK") ) 
    {
      Delayms(1000);
      TM_USART_Puts(USART1,"AT+CCLK?\r\n");
    }
    else
      printf("Nie skonfigurowano modemu");
  }
  else printf("Nie mozna uruchomic GSM");
  
  for (int i=0;i<sizeof tmp;i++) tmp[i]=TM_USART_Getc(USART1);
  token = strtok (tmp,"\"");
  printf("odebrano ciag o wartosci: %s \r\n",token);
